...
class CapsuleSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        self.startState = (startingGameState.getPacmanPosition(), startingGameState.getFood(), startingGameState.getCapsules()[0])
        self.startingGameState = startingGameState
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if state[2]:
                if (nextx, nexty) == state[2]:
                    return [(((nextx, nexty), state[1], False), direction, 1)]
                else:
                    if not self.walls[nextx][nexty] and not state[1][nextx][nexty]:
                        successors.append((((nextx, nexty), state[1], state[2]), direction, 1))
            else:
                if not self.walls[nextx][nexty]:
                    nextFood = state[1].copy()
                    nextFood[nextx][nexty] = False
...
def capsuleProblemHeuristic(state, problem):
    ...
    max_y = 0
    if state[2]:
        return manhattanDistance(state[0], state[2])
    else:
        if state[1].asList():
            for food in state[1].asList():
                curr_x = abs(state[0][0] - food[0])
                curr_y = abs(state[0][1] - food[1])
                if curr_x > max_x:
                    max_x = curr_x
                if curr_y > max_y:
                    max_y = curr_y
